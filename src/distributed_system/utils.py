...
from src.constants import TRAFFIC_LIGHTS, LIGHTS_TIME_COUNT

def setup_gpio_output_pins(pins: list) -> None:
...
def turn_on_night_mode() -> None:
    ...
    setup_gpio_output_pins(
        [
            c_1_main_yellow_light_pin,
            c_1_aux_yellow_light_pin,
            c_2_main_yellow_light_pin,
            c_2_aux_yellow_light_pin,
        ]
    )
    while True:
        if datetime.now() >= execution_time_min:
